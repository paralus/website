...
Kubernetes as we all know is the most widely used container orchestration tool with a lot of moving parts. Nodes, clusters, pods, services, replica sets and a lot of other things.

Logging activities across all these elements isn't an easy task, especially due to its ephemeral and dynamic nature.  With such a nature, Kubernetes provides three different types of logs:
...
- **Command Logs:** All the kubectl commands executed by an authorized user from the prompt via the dashboard are recorded under command logs. If a user connects to Paralus using `pctl`, the commands will also be recorded.

- **API Logs:** All the Kubernetes API calls are recorded under the API logs. If a user has configured the kubeconfig file and performs operations through their terminal, these actions will be recorded under API logs.
